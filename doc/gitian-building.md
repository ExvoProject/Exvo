...
*Setup instructions for a gitian build of Exvo using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Exvo
...
Get the [Debian 7.4 net installer](http://cdimage.debian.io/debian-cd/7.4.0/amd64/iso-cd/debian-7.4.0-amd64-netinst.iso).
...
[1] http://www.chiark.greenend.io.uk/~sgtatham/putty/download.html
...
Clone the git repositories for exvo and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/exvo/exvo
```
...
In [doc/release-process.md](release-process.md) in the exvo repository under 'Fetch and build inputs'.
...
**Note**: Do not fioet to copy the result from `build/out` to `inputs` after every gbuild command! This will save
you a lot of time.
...
To build Exvo (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the exvo repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/exvo/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/exvo/exvo
...
For example:
```bash
URL=https://github.com/laanwj/exvo.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit exvo=${COMMIT} --url exvo=${URL} ../exvo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit exvo=${COMMIT} --url exvo=${URL} ../exvo/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/exvo-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/exvo-build.assert
```
...
[exvo/gitian.sigs](https://github.com/exvo/gitian.sigs/) repository, or
