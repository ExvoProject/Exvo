...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            CExvoAddress address(donationAddress);
            if (!address.IsValid()) {
                strErr = "Invalid Exvo address in masternode.conf line: " + line;
                streamConfig.close();
                return false;
            }
        }
        ...
        if(port != 8585 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (must be 8585 for mainnet)";
            streamConfig.close();
            return false;
        }
        else if(port != 8565 && Params().NetworkIDString() == CBaseChainParams::TESTNET) {
            strErr = "Invalid port detected in masternode.conf: " + line + " (must be 8565 for testnet)";
            streamConfig.close();
            return false;
        }
