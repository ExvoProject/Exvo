...
const char * const EXVO_CONF_FILENAME = "exvo.conf";
const char * const EXVO_PID_FILENAME = "exvo.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
				...
				/**TODO-- */
				// "exvo" is a composite category enabling all Exvo-related debug output
				if(ptrCategory->count(string("exvo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\exvo
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\exvo
    // Mac: ~/Library/Application Support/exvo
    // Unix: ~/.exvo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "exvo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/exvo";
#else
    // Unix
    return pathRet / ".exvo";
#endif
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
		/**TODO-- */
        // Create empty exvo.conf if it does not excist
        //FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        //if (configFile != NULL)
            //fclose(configFile);
        return; // Nothing to read, so just return //return; // No exvo.conf file is OK
    }
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override exvo.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", EXVO_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Exvo Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Exvo Core") == std::string::npos) {
