 ...
 * between exvod, and exvo-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), EXVO_CONF_FILENAME));
    if (mode == HMM_EXVOD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), EXVO_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/exvo/exvo>";
    const std::string URL_WEBSITE = "<https://exvo.io>";

	//TODO-- add exvo license
    return FormatParagraph(strprintf(_("Copyright (C) 2009 -%i The Bitcoin Core developers"), COPYRIGHT_YEAR)) + "\n" +
		   FormatParagraph(strprintf(_("Copyright (C) 2014 -%i The Dash Developers"), COPYRIGHT_YEAR)) + "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The EXVO Core Developers"), COPYRIGHT_YEAR)) + "\n" + "\n" +
...
/** Sanity checks
 *  Ensure that Exvo is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single exvo process is using the data directory.
    ...
    try {
       ...
       // Wait maximum 10 seconds if an old wallet is still running. Avoids lockup during restart
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Exvo is probably already running."), strDataDir));
        if (probeOnly) {
            lock.unlock();
        }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", EXVO_CONF_FILENAME)).string());
	...
	if (mapMultiArgs.count("-masternodepaymentskey")) // masternode payments priv key
    {
        if (!masternodePayments.SetPrivKey(GetArg("-masternodepaymentskey", ""))){
			...
			//LogPrintf("Masternode: Create new key %s", CExvoSecret(secret).ToString());
   ...
   //if (!fDisableWallet) {
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Exvo was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Exvo with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty()) {
        return InitError(_("Exvo was not built with wallet support. Set -mineraddress, or rebuild Exvo with wallet support."));
    }
 #endif // !ENABLE_WALLET

    if (mapMultiArgs.count("-mineraddress")) {
 #ifdef ENABLE_WALLET
        bool minerAddressInLocalWallet = false;
        if (pwalletMain) {
            // Address has alreday been validated
            CExvoAddress addr(GetArg("-mineraddress",""));
            CKeyID keyID;
            addr.GetKeyID(keyID);
    ...
    fProUserModeDarksendInstantX = GetBoolArg("-promode", false); //im Standart an (Darksend und Instantx ist im QT nicht sichtbar)
	fProUserModeDarksendInstantX2 = GetBoolArg("-disable_DS_InstantX", false);  //im Standart aus (Darksend und Instantx ist im Core an)
