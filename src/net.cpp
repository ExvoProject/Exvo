...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Exvo users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Exvo " + FormatFullVersion();
        ...
        std::set<std::vector<unsigned char> > setConnected;
        {
            ...
            BOOST_FOREACH(CNode* pnode, vNodes) {
                if (!pnode->fInbound && !pnode->fAddnode) {
                    ...
                    if (pnode->fSuccessfullyConnected && !pnode->fFeeler 
...
#ifndef WIN32
#ifdef SO_NOSIGPIPE
    // Different way of disabling SIGPIPE on EXVO
...
void CConnman::AddNewAddress(CAddress& addr, const CAddress& addrFrom, int64_t nTimePenalty)
{
