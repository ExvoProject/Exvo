...
#ifndef EXVO_VALIDATION_H
#define EXVO_VALIDATION_H
...
static const unsigned int DEFAULT_MIN_RELAY_TX_FEE = 10000; //TODO--
//! -maxtxfee default
...
static const int64_t DEFAULT_MAX_TIP_AGE = 6 * 60 * 60; //~360 blocks
...
extern std::map<uint256, int64_t> mapRejectedBlocks;//TODO--
...
static const unsigned int MIN_BLOCKS_TO_KEEP = 100;

static const signed int DEFAULT_CHECKBLOCKS = 6;
static const unsigned int DEFAULT_CHECKLEVEL = 3;

// Require that user allocate at least 699MB for block & undo files (blk???.dat and rev???.dat)
// At 4MB per block, 100 blocks = 400MB.
// Add 15% for Undo data = 460MB
// Add 20% for Orphan block rate = 552MB
// We want the low water mark after pruning to be at least 552MB and since we prune in
// full block file chunks, we need the high water mark which triggers the prune to be
// one 128MB block file + added 15% undo data = 147MB greater for a total of 699MB
// Setting the target to > than 550MB will make it likely we can respect the target.
static const uint64_t MIN_DISK_SPACE_FOR_BLOCK_FILES = 699 * 1024 * 1024;
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime);
...
/** TODO-- Reprocess a number of blocks to try and get on the correct chain again **/
