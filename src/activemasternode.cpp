...
//
// Bootup the Masternode, look for a 7500 EXVO input and register on the network
//
...
{	
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(Params().NetworkIDString() == "main"){
            if(service.GetPort() != 8585) {
                notCapableReason = "Invalid port: " + boost::lexical_cast<string>(service.GetPort()) + " - only 8585 is supported on mainnet.";
                status = MASTERNODE_NOT_CAPABLE;
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason.c_str());
                return;
            }
        } else if(service.GetPort() == 8585) {
            notCapableReason = "Invalid port: " + boost::lexical_cast<string>(service.GetPort()) + " - 8585 is only supported on mainnet.";
            status = MASTERNODE_NOT_CAPABLE;
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason.c_str());
            return;
        }
		...
		* This is the expected.
				if(g_connman->OpenNetworkConnection((CAddress)service, false, NULL, service.ToString().c_str())){
                    //todo++ object added
		...
		**/
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in exvo.conf */
...
bool CActiveMasternode::Register(CTxIn vin, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, CScript donationAddress, int donationPercentage, std::string &retErrorMessage) {
    ...
    if(pmn == NULL)
    {
        LogPrintf("** FAILED TO FIND NODE: CActiveMasternode::Register() - Adding to Masternode list service: %s - vin: %s\n", service.ToString().c_str(), vin.ToString().c_str());
    }
