...
BOOST_AUTO_TEST_CASE(d106_difficulty_test) {
    SelectParams(CBaseChainParams::MAIN);
    const Consensus::Params &params = Params().GetConsensus();
    ...
    CBlockHeader blkHeaderDummy;
    uint32_t nBits =
        GetNextWorkRequiredD106(&blocks[2049], &blkHeaderDummy, params);

    // Difficulty stays the same as long as we produce a block every 10 mins.
    for (size_t j = 0; j < 10; i++, j++) {
        blocks[i] = GetBlockIndex(&blocks[i - 1], 600, nBits);
        BOOST_CHECK_EQUAL(
            GetNextWorkRequiredD106(&blocks[i], &blkHeaderDummy, params),
            nBits);
    }
    ...
    BOOST_CHECK_EQUAL(
        GetNextWorkRequiredD106(&blocks[i++], &blkHeaderDummy, params), nBits);
    ...
    // impact.
    blocks[i] = GetBlockIndex(&blocks[i - 1], 6000, nBits);
    nBits = GetNextWorkRequiredD106(&blocks[i++], &blkHeaderDummy, params);
