...
#if defined(NDEBUG)
# error "Exvo cannot be compiled without assertions."
#endif

/**
 * Global state
 */
#define PRE_SUBSIDY_HEIGHT 500
...
const std::string strMessageMagic = "Exvo Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fDryRun)//TODO--
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s EXVO additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Initial dev fund, airdrop, donations
    if (nHeight == 1)
        nSubsidy = 3000000 * COIN;
    // Instant mine protection
    else if (nHeight > 1 && nHeight <= PRE_SUBSIDY_HEIGHT)
        nSubsidy = 1 * COIN;
    // Hard limit - 60,000,000 EXVO
    else if (nHeight > 5700000)
        nSubsidy = 0;
...
public:
    ...
    bool AuthoritativeDeployment() const { return false; };
};
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if(pindex->nHeight > chainparams.GetConsensus().MasternodePaymentStartHeight + 7475)
    {
        ...
        else
        {
            ...
            CExvoAddress address(txdestaddr);
    ...
    //LogPrintf("tracking money supply: nValueOut: %d, nValueIn: %d, nFees: %d, nMint: %d\n",
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
        ...
        int64_t cacheSize = pcoinsTip->DynamicMemoryUsage() * DB_PEAK_USAGE_FACTOR; // Compensate for extra memory peak (x1.5-x1.9) at flush time.
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // check if masternode payments have been activated
