...
###   This script attempts to download the signature file SHA256SUMS.asc from exvo.org
...
WORKINGDIR="/tmp/exvo_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://exvo.org/bin/"
VERSIONPREFIX="exvo-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'exvo-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Exvo Core binary release signing key installed?"
   fi
