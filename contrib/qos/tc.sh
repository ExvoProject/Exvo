# Copyright (c) 2017 The Exvo Core developers
...
LINKCEIL="1gbit"
#limit outbound Exvo protocol traffic to this rate
LIMIT="160kbit"
...
iptables -t mangle -A OUTPUT -p tcp -m tcp --dport 8585 ! -d ${LOCALNET_V4} -j MARK --set-mark 0x2
iptables -t mangle -A OUTPUT -p tcp -m tcp --sport 8585 ! -d ${LOCALNET_V4} -j MARK --set-mark 0x2

if [ ! -z "${LOCALNET_V6}" ] ; then
	ip6tables -t mangle -A OUTPUT -p tcp -m tcp --dport 8585 ! -d ${LOCALNET_V6} -j MARK --set-mark 0x4
	ip6tables -t mangle -A OUTPUT -p tcp -m tcp --sport 8585 ! -d ${LOCALNET_V6} -j MARK --set-mark 0x4
fi
